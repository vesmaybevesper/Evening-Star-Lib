...
public class FastJsonConfigSerializer<T> extends ConfigSerializer<T> {
	...
	private static Tag jsonValueToTag(Object value) {
		...
		//?} 1.20.1{
		/*if (value instanceof Number number) {
			if (number instanceof Integer i) {return IntTag.valueOf(i);}
			if (number instanceof Long l) {return LongTag.valueOf(l);}
			if (number instanceof Double d) {return DoubleTag.valueOf(d);}
			if (number instanceof Float f) {return FloatTag.valueOf(f);}
			return LongTag.valueOf(number.longValue());
		}
		*///?}
